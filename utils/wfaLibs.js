...
let initLibsCache = async () => {
    ...
    // 自动繁转简：遍历 Nyx 词库，将所有繁体中文转为简体
    try {
        const TRAD_TO_SIMP = {
            '體':'体','為':'为','電':'电','擊':'击','護':'护','戰':'战','鬥':'斗',
            '滅':'灭','殺':'杀','劍':'剑','動':'动','禦':'御','飛':'飞','導':'导',
            '彈':'弹','發':'发','關':'关','機':'机','門':'门','隊':'队','團':'团',
            '處':'处','準':'准','點':'点','槍':'枪','鏈':'链','潛':'潜','襲':'袭',
            '衝':'冲','鋒':'锋','偵':'侦','測':'测','隱':'隐','視':'视','鏡':'镜',
            '熱':'热','聲':'声','響':'响','頻':'频','鎖':'锁','標':'标','識':'识',
            '譯':'译','監':'监','聽':'听','獵':'猎','蹤':'踪','跡':'迹','軌':'轨',
            '躍':'跃','攔':'拦','截':'截','擴':'扩','張':'张','壓':'压','縮':'缩',
            '變':'变','換':'换','轉':'转','連':'连','接':'接','斷':'断','開':'开',
            '閉':'闭','創':'创','廢':'废','棄':'弃','尋':'寻','獲':'获','靈':'灵',
            '資':'资','源':'源','訊':'讯','號':'号','圖':'图','複':'复','製':'制',
            '藍':'蓝','裝':'装','備':'备','構':'构','築':'筑','設':'设','劃':'划',
            '計':'计','畫':'画','評':'评','價':'价','購':'购','買':'买','賣':'卖',
            '庫':'库','儲':'储','網':'网','絡':'络','協':'协','調':'调','驗':'验',
            '實':'实','報':'报','紀':'纪','錄':'录','統':'统','顯':'显','選':'选',
            '項':'项','數':'数','據':'据','檔':'档','編':'编','輯':'辑','刪':'删',
            '確':'确','認':'认','幫':'帮','說':'说','錯':'错','誤':'误','範':'范',
            '圍':'围','內':'内','後':'后','層':'层','級':'级','優':'优','隨':'随',
            '權':'权','預':'预','設':'设','默':'默','啟':'启','暫':'暂','時':'时',
            '週':'周','期':'期','長':'长','遠':'远','近':'近','強':'强','弱':'弱',
            '徑':'径','域':'域','區':'区','與':'与','並':'并','從':'从','見':'见',
            '觀':'观','選':'选','擇':'择','優':'优','化':'化',
        }
        const keys = libs.Nyx.keys()
        let converted = 0
        for (const k of keys) {
            const entry = libs.Nyx.get(k)
            if (!entry || !entry.zh) continue
            let changed = false
            const arr = [...entry.zh]
            for (let i = 0; i < arr.length; i++) {
                const simp = TRAD_TO_SIMP[arr[i]]
                if (simp) { arr[i] = simp; changed = true }
            }
            if (changed) { entry.zh = arr.join(''); converted++ }
        }
        if (converted) logger.info(`自动繁转简: ${converted} 条`)
    } catch (e) {
        logger.warn(`自动繁转简失败: ${(e && e.message) || e}`)
    }
