...
// 加载黑话别名
try {
  const aliasPath = path.join(__dirname, "../api/dataSource/alias_snapshot.json");
  ...
  logger.info("[warframes] 黑话别名: " + Object.keys(aliasMap).length + " 条");
} catch (e) { logger.warn("[warframes] 黑话别名加载失败: " + e.message); }
